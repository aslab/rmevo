...
class Params:
    ...
    def to_sdf(self, node_elem):
        if self.period is not None:
            pyrmevo.SDF.sub_element_text(node_elem, 'rv:period', self.period)
        if self.phase_offset is not None:
            pyrmevo.SDF.sub_element_text(node_elem, 'rv:phase_offset', self.phase_offset)
        if self.amplitude is not None:
            pyrmevo.SDF.sub_element_text(node_elem, 'rv:amplitude', self.amplitude)
        if self.bias is not None:
            pyrmevo.SDF.sub_element_text(node_elem, 'rv:bias', self.bias)
        if self.gain is not None:
            pyrmevo.SDF.sub_element_text(node_elem, 'rv:gain', self.gain)
